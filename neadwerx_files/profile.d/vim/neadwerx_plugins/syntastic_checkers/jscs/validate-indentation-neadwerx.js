...
module.exports.prototype = {
    ...
    getOptionName: function() {
        return 'validateIndentationNeadwerx';
    },
    ...
    check: function(file, errors) {
        ...
        function checkIndentations() {
            linesToCheck.forEach(function(line, i) {
                ...
                var expectedIndentation = getExpectedIndentation(line, actualIndentation, i);
        ...
        function getExpectedIndentation(line, actual, i) {
            ...
            if (line.pop === null) {
                // check if line above matches this line at columns
                if( i >= 1 )
                {
                    var rNotIndentChar = new RegExp('[^' + indentChar + ']');
                    var rNotVariableChar = new RegExp('[^A-Za-z0-9_]');

                    var firstContent = Math.max(lines[i].search(rNotIndentChar), 0);
                    var prev_firstContent = Math.max(lines[i-1].search(rNotIndentChar), 0);

                    var line_sub_string = lines[i].substring(firstContent);

                    var lastVariableContent = line_sub_string.search(rNotVariableChar);

                    var start_col = firstContent;
                    var end_col = lastVariableContent + firstContent - 1;
                    var current_line_split = lines[i].split('');
                    var prev_line_split = lines[i-1].split('');

                    var content_matches = true;
                    var content_matches_4 = false;
                    for( var j = start_col; j < end_col; j++ )
                    {
                        if( 
                             j >= current_line_split.length ||
                             j >= prev_line_split.length
                          )
                        {
                            // error
                            content_matches = false;
                            content_matches_4 = false;
                            break;
                        }

                        content_curr = current_line_split[j];
                        content_prev = prev_line_split[j];

                        if( content_curr !== content_prev )
                        {
                            if( j+4 < prev_line_split.length )
                            {
                                // try 4 + j "var "
                                content_prev_4 = prev_line_split[j+4];

                                if( content_curr === content_prev_4 )
                                {
                                    content_matches_4 = true;
                                }
                                else
                                {
                                    // error
